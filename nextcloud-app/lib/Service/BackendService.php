...
class BackendService
{
    ...
    /**
     * The user-tunable losos.* settings parsed from overrides.nix by the
     * backend, for the admin page's first paint. Keys: sharingMyStorage,
     * nextcloudMode, forgejoMode, hostName, https, gpuEnable, aioApachePort,
     * aioInterfacePort (see backend/schema.json settingsResponse).
     */
    public function getSettings(): array
    {
        return $this->callJson(['settings', '--json']);
    }

    /**
     * Apply a full overrides.nix body (generated by the admin page from the
     * form) and trigger a rebuild. The Nix code is piped to `losos-ctl apply`
     * on stdin — the backend owns validation + the atomic file rewrite +
     * spawning nixos-rebuild. Returns the rebuild job id.
     * @return array{job: string}
     */
    public function apply(string $nixCode): array
    {
        return $this->callJson(['apply'], $nixCode);
    }
     ...
     * @param string|null $stdin optional bytes piped to the subprocess (apply)
     */
    private function callJson(array $args, ?string $stdin = null): array
    {
        ...
        [$stdout, $stderr, $exit] = $this->runner->run($argv, $stdin);
